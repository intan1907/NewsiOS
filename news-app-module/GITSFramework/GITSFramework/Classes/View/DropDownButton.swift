...
//  GITSFramework
//
//  Created by Alvira Nurhaliza Wijaya on 07/03/19.
...
public class DropDownButton: UIButton {
    ...
    @IBInspectable var borderColor: UIColor = UIColor("#CDD6DF")! //default border colors
    var title: String = ""
    ...
    override public func layoutSubviews() {
        ...
        let buttonContentInsets = self.contentEdgeInsets
        self.titleLabel?.frame.origin.x = buttonContentInsets.left
        self.imageView?.frame.origin.x = self.frame.size.width - (self.imageView?.frame.size.width ?? 0) - buttonContentInsets.right
        self.titleLabel?.frame.size.width = self.frame.size.width - buttonContentInsets.left - buttonContentInsets.right - (self.imageView?.frame.size.width ?? 0) - self.imageEdgeInsets.left
    }
